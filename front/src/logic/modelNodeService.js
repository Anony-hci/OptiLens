...
import { currentPreference, getProblemModel, getConstraintsChanges, getObjectivesChanges, getObjectives, getConstraints, clearModifiedPanel, addFilteredConstraintsToConstraints } from './preferenceService';
...
export class ModelNode {
  constructor(id) {
    ...
    this.coursesChanges = []; // 添加coursesChanges属性
...
export const features = computed(() => {
  ...
  if (currentPreference.value && 
      currentPreference.value.solutionResults && 
      currentPreference.value.solutionResults.solutions && 
      currentPreference.value.solutionResults.solutions.length > 0 &&
      currentPreference.value.solutionResults.solutions[0].features) {
...
export const getFeatureDisplay = (featureName) => {
  ...
  if (maps.hasOwnProperty(featureName)) {
    ...
    // 如果featureName过长则截断，超出部分用省略号表示
    const maxLen = 100; // 可根据需要调整最大长度
    if (featureName.length > maxLen) {
      return featureName.slice(0, maxLen) + '...';
    }
    return featureName;
  }
}
...
export const addNewNode = (isSolution = false, addFeatures = false, featureNames = [], isCandidates = false) => {
    // 创建新节点
    ...
    newNode.coursesChanges = JSON.parse(JSON.stringify(currentPreference.value.coursesChanges || [])); // 复制coursesChanges
    newNode.currentSolutionIndex = currentPreference.value.currentSolutionIndex
    newNode.isSolution = isSolution;
    if(isSolution){
      newNode.type = "solution";
    }else if(isCandidates){
      newNode.type = "candidates";
    }else{
...
const generateAnnotation = (objectives_changes, constraints_changes, courses_changes) => {
  ...
  if (objectives_changes.length > 0) {
    objectives_changes.forEach(change => {
      if (change.type === 'add') {
        changes.push(`添加目标：${change.name}`);
      } else if (change.type === 'delete') {
        changes.push(`删除目标：${change.old_name} -> ${change.name}`);
      } else if (change.type === 'update') {
        changes.push(`修改目标：${change.old_name} -> ${change.name}`);
      }
    });
  }
...
export const solving = async () => {
    ...
    addFilteredConstraintsToConstraints();
    clearModifiedPanel();
...
export const switchActiveNode = (node) => {
  ...
  logUserAction(ACTION_TYPES.SWITCH_NODE, {
    node_id: currentPreference.value.father_id,
    node_type: modelNodes.value[currentPreference.value.father_id].type
  })
