...
export const updateMessagesToBackend = async (messages) => {
  try {
    ...
    console.log('Messages updated successfully:', messages);
    return data;
  } catch (error) {
...
// 保存数据到后端（消息和偏好设置）
export const saveDataToBackend = async (messages, currentPreference, modelNodes = []) => {
  try {
    const sessionId = localStorage.getItem('sessionId');
    if (!sessionId) {
      throw new Error("Session ID not found.");
    }
    const data = await fetchData(`${API_URL}/save_data`, 'POST', { 
      messages, 
      currentPreference,
      modelNodes,
      sessionId 
