...
import { addFindMessage, addUserMessage, generateMessageId, messages, processResponseMessage } from './messageService';
...
import { toggleshowAllCourses, getCourseKey, courseDisplayByKey } from './scheduleService';
...
watch(filteredSolutions, () => {
  ...
  updateCandidateItems();
  for (const item of currentPreference.value.candidateItems) {
    const courseKey = getCourseKey(item);
    if(item.chosen){
      courseDisplayByKey.value[courseKey] = item.chosen;
    }
  }
});
...
export const confirmSolution = async() => {
  ...
  // 记录保存课表操作 - 使用新的ACTION_TYPE
  const selectedCourses = currentPreference.value.candidateItems.filter(item => item.chosen);
  logUserAction(ACTION_TYPES.SAVE_COURSES, {
    course_list: selectedCourses.map(item => ({
...
// 新增：支持选择特定课程的保存函数
export const confirmSolutionWithSelectedCourses = async(selectedCourses) => {
  // 记录保存课表操作 - 使用新的ACTION_TYPE
  logUserAction(ACTION_TYPES.SAVE_COURSES, {
    course_list: selectedCourses.map(item => ({
      course_name: item['课程名'],
  ...
  // 修复1：使用数组展开语法，并去重
  const confirmedCourseList = [
    ...selectedCourses, 
    ...currentPreference.value.candidateItems.filter(item => item.chosen_when_confirmed && item.chosen)
  ];
  
  // 去重：基于课程名、主讲教师、上课时间的组合去重
  const uniqueConfirmedCourseList = confirmedCourseList.filter((course, index, self) => {
    const courseKey = `${course['课程名']}_${course['主讲教师']}_${course['上课时间']}`;
    return index === self.findIndex(c => 
      `${c['课程名']}_${c['主讲教师']}_${c['上课时间']}` === courseKey
    );
  });
  
  console.log("confirmedCourseList", confirmedCourseList);
  console.log("uniqueConfirmedCourseList", uniqueConfirmedCourseList);
  const solutions = filteredSolutions.value.filter(solution => {
    return uniqueConfirmedCourseList.every(course => { 
      // 构建变量key
      const key = `x_${course['课程名']}_${course['主讲教师']}_${course['上课时间']}`;
      // 检查该课程在解决方案中的变量值是否为1
      return solution.Variables && solution.Variables[key] === 1;
    });
  });
  ...
  // 因为filteredSolutions变化了，所以需要updateCandidateItems
  updateCandidateItems();
  
  updatePreferenceSolutions(newSolutionResults);
  
  // 修复3：确保 confirmedCourseList 是数组
  currentPreference.value.candidateItems.forEach(item => {
    for (const course of uniqueConfirmedCourseList) {
      if (course['课程名'] === item['课程名'] && 
          course['主讲教师'] === item['主讲教师'] && 
          course['上课时间'] === item['上课时间']) {
  ...
  let description = `Save ${selectedCourses.length} courses: <br>${selectedCourses.map(course => `${course['课程名']}（${course['主讲教师']}）-${course['上课时间']}`).join('<br>')}`;
  
  // 修复4：使用 addUserMessage 替代直接操作 messages
  messages.value.push({
...
watch(filteredSolutions, () => {
  if(solutionsNum.value === 500 && filteredSolutionsNum.value < 500){
    addFindMessage('Since only 500 solutions were returned previously, not all matching solutions are shown.')
  }
})
...
export const updateCandidateItems = () => {
  ...
  if (currentSolution) {
    ...
    // 在更新完所有 chosen 状态后，同步更新 courseDisplayByKey
    // 这样可以确保 courseDisplayByKey 反映最新的 chosen 状态
    // currentPreference.value.candidateItems.forEach(item => {
    //   const courseKey = getCourseKey(item);
    //   courseDisplayByKey.value[courseKey] = item.chosen;
    // });
...
export const applyFilter = () => {
  ...
  console.log("applyFilter", solutions.value, filters.value)
  // 第一步：根据 filters 进行筛选
  let filtered = solutions.value.filter(solution => {
    return Object.keys(filters.value).every(filterKey => {
      ...
      if (filter.operation === 'equal') {
        ...
        return solution.features[filterKey] !== filter.value;
      } else if (filter.operation === 'greaterThanOrEqual' || filter.operation === 'greaterThan') {
        return solution.features[filterKey] >= filter.value;
      } else if (filter.operation === 'lessThanOrEqual' || filter.operation === 'lessThan') {
        return solution.features[filterKey] <= filter.value;
