...
.constraint-button {
  ...
  display: block; /* 改为block以便margin-left: auto生效 */
  font-size: 14px;
  cursor: pointer;
  border-radius: 5px;
}
...
/* userSelected 样式保持不变 */
.course-button.userSelected:not(.blocked) {
  border: 2px solid #1a73e8 !important;
}

.course-button.userSelected.infeasible {
  border: 1px solid #ccc;
}
...
.course-button.fixed, .course-favorite.fixed{
  background-color: #003366e4 !important; /* 背景黑色 */
  color: #fff; /* 字体白色 */
}
...
/* 移除 added 和 deleted 的背景色样式，改用标签 */
...
.status-tag {
  ...
  line-height: 1;
  color: white;
  pointer-events: none; /* 防止标签影响按钮点击 */
}
