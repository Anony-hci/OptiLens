    ...
    <div class="nodes-container" ref="nodesContainer">
      ...
      <div v-for="node in modelNodes" 
           ...
           :class="{ 
             ...
             'candidates-node': node.type === 'candidates'
           }"
        ...
        <div v-else class="node-container"  @click="handleNodeClick(node)">
          <div class="node-header">
            <div class="header-left" >
              {{ node.type === "solution" ? `[${node.id}] Saved Timetable` : node.type === "candidates" ? `[${node.id}] Modified Candidates` : `[${node.id}] Preference` }}
            </div>
          ...
          <div class="node-details">
            ...
            <!-- Solution 类型：显示选择的课程和特征取值 -->
            <div v-if="node.type === 'solution'" class="model-section">
              <div v-if="node.solutionResults && node.solutionResults.solutions && node.solutionResults.solutions.length > 0">
                <div><strong>Selected Courses：</strong></div>
                <div v-for="(course, index) in getSelectedCoursesFromSolution(node.solutionResults.solutions[0])" 
                     :key="course" 
                     style="padding-left: 20px;">
                  {{ index + 1 }}. {{ course }}
                </div>
                
                <div style="margin-top: 15px;"><strong>Statistics：</strong></div>
                <div v-for="(value, featureName) in node.solutionResults.solutions[0].features" 
                     :key="featureName"
                     style="padding-left: 20px;">
                  {{ getFeatureDisplay(featureName) }}: {{ value }}
                </div>
              </div>
              <div v-else style="padding-left: 20px; color: #666;">
                暂无求解结果
              </div>
            </div>

            <!-- AddedItem 类型：显示candidateItems中的具体课程名称（去重） -->
            <div v-else-if="node.type === 'candidates'" class="model-section">
              
              <!-- 显示课程变更详情 -->
              <div v-if="node.coursesChanges && node.coursesChanges.length > 0">
                <!-- 新增的课程 -->
                <div v-if="getAddedCourses(node.coursesChanges).length > 0">
                  <div style="margin-top: 10px;"><strong>Added Courses：</strong></div>
                  <div v-for="(course, index) in getAddedCourses(node.coursesChanges)" 
                       :key="course" 
                       style="padding-left: 20px;">
                    {{ index + 1 }}. {{ course }}
                  </div>
                ...
                <!-- 删除的课程 -->
                <div v-if="getRemovedCourses(node.coursesChanges).length > 0">
                  <div style="margin-top: 10px;"><strong>Deleted Courses：</strong></div>
                  <div v-for="(course, index) in getRemovedCourses(node.coursesChanges)" 
                       :key="course" 
                       style="padding-left: 20px;">
                    {{ index + 1 }}. {{ course }}
                  </div>
              ...
              <div style="margin-top: 15px;">
                <div><strong>Total Courses：{{ getUniqueCourseNames(node.candidateItems).length }}</strong></div>
                <div style="padding-left: 20px; margin-top: 5px;">
                  {{ getUniqueCourseNames(node.candidateItems).join('，') }}
                </div>
              </div>
            </div>

            <!-- Preference 类型：显示必修课、目标函数和约束 -->
            <div v-else class="model-section">
              <!-- 必修课程 -->
              <div><strong>Required Courses：</strong></div>
              <div v-if="node.requiredCourses && node.requiredCourses.length > 0">
                <div v-for="(course, index) in node.requiredCourses" 
                     :key="course" 
                     style="padding-left: 20px;">
                  {{ index + 1 }}. {{ course }}
                </div>
              ...
              <!-- 目标设置 -->
              <div style="margin-top: 15px;"><strong>Objectives：</strong></div>
              <div v-if="node.objectives && node.objectives.length > 0">
                <div v-for="(objective, index) in node.objectives"
              ...
              <!-- 约束设置 -->
              <div style="margin-top: 15px;"><strong>Constraints：</strong></div>
              <div v-if="node.constraints && node.constraints.length > 0">
                <div v-for="(constraint, index) in node.constraints"
                     :key="constraint.description"
                     style="padding-left: 20px;">
                  {{ index + 1 }}. {{ getConstraintDisplayText(constraint) }}
                </div>
              </div>
              <div v-else style="padding-left: 20px; color: #666;">
                无约束设置
        ...
        <div v-if="false && currentPreference.filteredConstraints.length > 0 || currentPreference.constraintsChanges.length > 0 || currentPreference.objectivesChanges.length > 0" class="model-section">
            ...
            >
                ...
                <span class="filter-text">{{ getFeatureDisplay(constraint.name) }}</span>
                ...
                {{ formatConstraintLhs(constraint.lhs) }} {{ constraint.constraint_type }} {{ constraint.rhs }}
                </div>
            </div>
...
// 格式化约束的 lhs 部分，处理新旧两种格式
const formatConstraintLhs = (lhs) => {
  if (!lhs) {
    return '';
  }
  
  // 新格式：对象包含 template_id 和 parameters
  if (typeof lhs === 'object' && lhs.template_id) {
    const templateInfo = `模板${lhs.template_id}`;
    if (lhs.parameters && Object.keys(lhs.parameters).length > 0) {
      const params = Object.entries(lhs.parameters)
        .map(([key, value]) => `${key}: ${Array.isArray(value) ? value.join(', ') : value}`)
        .join(', ');
      return `${templateInfo}(${params})`;
    }
    return templateInfo;
  }
...
// 从solution的Variables中提取选择的课程
const getSelectedCoursesFromSolution = (solution) => {
  if (!solution || !solution.Variables) return [];
  ...
  for (const [varName, value] of Object.entries(solution.Variables)) {
    if (value === 1) {
      ...
      if (parts.length >= 4) {
        ...
        const time = parts.slice(3).join('_'); // 处理时间可能包含下划线的情况
        selectedCourses.push(`${courseName}（${teacher}）- ${time}`);
      }
    }
  }
  return selectedCourses.sort(); // 排序以便于阅读
};

// 从candidateItems中提取唯一的课程名称
const getUniqueCourseNames = (candidateItems) => {
  if (!candidateItems || !Array.isArray(candidateItems)) return [];
  
  const uniqueCourses = new Set();
  candidateItems.forEach(item => {
    if (item['课程名']) {
      uniqueCourses.add(`${item['课程名']}`);
    }
  });
  
  return Array.from(uniqueCourses).sort(); // 转换为数组并排序
};

// 获取约束的简洁显示文本
const getConstraintDisplayText = (constraint) => {
  ...
  if (constraint.name) {
    displayText = getFeatureDisplay(constraint.name);
  } else if (constraint.lhs) {
    displayText = formatConstraintLhs(constraint.lhs);
  }
  
  if (constraint.constraint_type && constraint.rhs !== undefined) {
    displayText += ` ${constraint.constraint_type} ${constraint.rhs}`;
  }
  
  return displayText || '未知约束';
};

// 从coursesChanges中提取新增的课程
const getAddedCourses = (coursesChanges) => {
  if (!coursesChanges || !Array.isArray(coursesChanges)) return [];
  const addedChanges = coursesChanges.filter(change => change.type === 'add');
  return getSelectedCourses(addedChanges);
};

// 从coursesChanges中提取删除的课程
const getRemovedCourses = (coursesChanges) => {
  if (!coursesChanges || !Array.isArray(coursesChanges)) return [];
  const removedChanges = coursesChanges.filter(change => change.type === 'remove');
  return getSelectedCourses(removedChanges);
};
...
.candidates-node .node-container {
  background: #f0f9f0;  /* 浅绿色背景 */
  border: 1px solid #b3e5b3;  /* 绿色边框 */
}

.candidates-node .node-header {
  color: #28a745;  /* 绿色标题 */
  border-bottom-color: #b3e5b3;  /* 绿色分隔线 */
}

.candidates-node.active .node-container {
  border-color: #28a745;
  box-shadow: 0 0 8px rgba(40, 167, 69, 0.4);
}

.candidates-node:hover .node-container {
